...
def get_keypoint(humans, hnum, peaks):
    ...
    human = humans[0][hnum]
    C = human.shape[0]
    global peak1,peak5, peak6, peak9, peak10
    for j in range(C):
        k = int(human[j])
        if k >= 0:
...
def execute(img, src, t):
    ...
    if peak5[1] != 0 and peak7[1] != 0 and peak9[1] != 0:
        ...
        cv2.putText("angle: %f" % (leftelbow_angle), (60, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 1)
    else:
        cv2.putText("Please Show your whole body", (60, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 1)
